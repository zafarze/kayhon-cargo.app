...
from logistics.models import ClientProfile, CompanySettings, ProhibitedItem, Package
...
def handle_contact(message):
    ...
    if created:
        unique_password = secrets.token_urlsafe(10)
...
def default_handler(message):
    ...
    try:
        ...
        if text == get_text(lang, 'btn_track'):
            ...
            inline.add(InlineKeyboardButton("📸 Instagram", url="https://instagram.com/kayhon_cargo"))

            import os as _os
            photo_path = _os.path.normpath(_os.path.join(settings.BASE_DIR, '..', 'frontend', 'public', 'bot', 'contacts.jpg'))

            try:
                with open(photo_path, 'rb') as photo_file:
                    bot.send_photo(message.chat.id, photo_file, caption=caption, parse_mode='Markdown', reply_markup=inline)
            ...
            import os as _os
            photo_path = _os.path.normpath(_os.path.join(settings.BASE_DIR, '..', 'frontend', 'public', 'bot', 'price.jpg'))

            try:
                with open(photo_path, 'rb') as photo_file:
                    bot.send_photo(message.chat.id, photo_file, caption=tariff_text, parse_mode='Markdown')
            except FileNotFoundError:
            ...
            if items:
                ...
                msg = (
                    ...
                    + "\n".join(lines) +
                    "\n\n━━━━━━━━━━━━━━━━━\n"
                    "⚠️ _Отправитель несёт полную ответственность за содержимое посылки. При обнаружении запрещённых товаров груз будет задержан и передан в соответствующие органы._"
                )
            else:
                msg = (
                    "🚫 *Запрещённые к перевозке товары:*\n"
